...
const TicketCard = ({ ticket }) => {
  ...
  const isCertificate =
    ticket.isCertificate || ticket.title.toLowerCase().includes("certificate");
    ...
    <>
      ...
      >
        ...
        <CardFooter className="flex-col items-start">
          ...
          {/* Show purchase date as the main date */}
          {ticket.purchaseDate ? (
            <p className="text-default-500">
              {ticket.purchaseDate.toLocaleDateString()} at{" "}
              {ticket.purchaseDate.toLocaleTimeString()}
            </p>
          ) : (
            <p className="text-default-500">{ticket.date}</p>
          )}
          <p className="text-default-500 text-small">{ticket.venue}</p>
          {/* Show event date as secondary info */}
          <p className="text-default-400 text-xs">Event: {ticket.date}</p>
          <div className="mt-2 flex gap-2">
            <Chip color="primary" variant="flat">
              {isCertificate
                ? `Certificate#${ticket.tokenId}`
                    ...
                    <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200">
                      <div className="text-center space-y-3">
                        ...
                        <p className="text-gray-700">
                          This certificate serves as proof that you have
                          successfully participated in and completed the
                          requirements for:
                        </p>
